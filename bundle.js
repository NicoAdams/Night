...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.subscribeKeyDownListener = subscribeKeyDownListener;
	exports.unsubscribeKeyDownListener = unsubscribeKeyDownListener;
	...
	function subscribeKeyDownListener(f) {
	  return keyDownListener.subscribe(f);
	}
	function unsubscribeKeyDownListener(i) {
	  return keyDownListener.unsubscribe(i);
	}

	var keyUpListener = (0, _listener_subscriber.listenerCreator)();
	window.onkeyup = keyUpListener.getListener();
	function subscribeKeyUpListener(f) {
	  return keyUpListener.subscribe(f);
	}
	function unsubscribeKeyUpListener(i) {
	  return keyUpListener.unsubscribe(i);
	}
	...
	function listenerCreator() {
		...
		var lc = {
			...
			unsubscribe: function unsubscribe(i) {
				if ((0, _lodash.has)(bindings, i)) {
					(0, _lodash.unset)(bindings, i);
					return true;
				}
				return false;
			},
			getListener: function getListener() {
				var printOut = arguments.length <= 0 || arguments[0] === undefined ? null : arguments[0];

				return function (event) {
					if (printOut) {
						console.log(printOut);
					}
					(0, _lodash.forEach)(bindings, function (f) {
						return f(event);
					});
				};
			}
...
/***/ function(module, exports, __webpack_require__) {
	...
	var viewport = {
		...
		setCenter: function setCenter(gameCoords) {
	...
	function vec(x, y) {
		var v = {
			...
			toString: function toString() {
				...
				var vToString = v.copy();
				if (roundTo) {
					vToString = v.map(function (i) {
						return (0, _util.round)(i, roundTo);
					});
				}
			...
			add: function add(v2) {
				return v.map2(v2, function (i, i2) {
					return i + i2;
				});
			},
			sub: function sub(v2) {
			...
			rotate: function rotate(angle) {
				return vec(v.x * Math.cos(angle) - v.y * Math.sin(angle), v.x * Math.sin(angle) + v.y * Math.cos(angle));
			},
			rotateAbout: function rotateAbout(angle, v2) {
				var vd = v.sub(v2);
				var vdRot = vd.rotate(angle);
				return v.sub(vd).add(vdRot);
			},
			limit: function limit(vLim) {
				return vec((0, _util.limit)(v.x, vLim.x), (0, _util.limit)(v.y, vLim.y));
			},
			unit: function unit() {
				var l = v.len();
				if (l == 0) {
			...
			project: function project(vProject) {
				var vProjectUnit = vProject.unit();
				return vProjectUnit.mul(v.dot(vProjectUnit));
			},
			projectScalar: function projectScalar(vProject) {
				return v.dot(vProject.unit());
			},
			normal: function normal() {
				return v.rotate(Math.PI / 2).unit();
			},
			mulAlong: function mulAlong(vAlong, n) {
...
/***/ function(module, exports) {
	...
	function round(n, exponent) {
		var base = Math.round(Math.pow(10, Math.abs(exponent)));
		if (exponent > 0) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var lightingObj2 = (0, _object.addLightingFunction)((0, _object.makeRegularPolyObject)(8, character.object.com(), 350, 0), function (obj) {
		return {
	...
	var lightingObj1 = (0, _object.addLightingFunction)((0, _object.makeRegularPolyObject)(8, character.object.com(), 250, 0), function (obj) {
		return {
	...
	function makePolyObject(points) {
		...
		var polyObject = {
			...
			area: function area() {
				...
				var total = 0;
				(0, _array_util.forEachPair)(polyObject.points, function (p1, p2) {
			...
			com: function com() {
				// https://en.wikipedia.org/wiki/Centroid
				var a = polyObject.area();
				var total = (0, _geom.vec)(0, 0);
				(0, _array_util.forEachPair)(polyObject.points, function (p1, p2) {
			...
			overlapsBoundingBox: function overlapsBoundingBox(objectOther) {
				var result = boundingBox.xRange.overlaps(objectOther.getBoundingBox().xRange) && boundingBox.yRange.overlaps(objectOther.getBoundingBox().yRange);
				return result;
			},
			projectOnto: function projectOnto(vProject) {
				if (polyObject.points.length == 0) {
					return null;
				}
				var r = null;
				(0, _array_util.forEachPair)(polyObject.points, function (p1, p2) {
					var project1 = p1.projectScalar(vProject),
					    project2 = p2.projectScalar(vProject);
					if (!r) {
						r = (0, _range.makeRange)(project1, project2);
					} else {
						var r2 = (0, _range.makeRange)(project1, project2);
						r = r.union(r2);
					}
				});
				return r;
			},
			getNormals: function getNormals() {
				var normals = [];
				(0, _array_util.forEachPair)(polyObject.points, function (p1, p2) {
			...
			getOwnProjections: function getOwnProjections() {
...
/***/ function(module, exports, __webpack_require__) {
	...
	function sum(arr) {
		...
		return (0, _lodash.reduce)(arr, function (a, b) {
			return a + b;
		});
	}
...
/***/ function(module, exports, __webpack_require__) {
	...
	function makeRange(n1, n2) {
		...
		var r = {
			min: min,
			max: max,
			round: function round(roundTo) {
				return makeRange((0, _util.round)(r.min, roundTo), (0, _util.round)(r.max, roundTo));
			},
			toString: function toString() {
				var roundTo = arguments.length <= 0 || arguments[0] === undefined ? null : arguments[0];
			...
			intersect: function intersect(r2) {
				if (!r.overlaps(r2)) {
					return null;
				}
				return makeRange(Math.max(r.min, r2.min), Math.min(r.max, r2.max));
			},
			union: function union(r2) {
				if (!r.overlaps(r2)) {
					return null;
				}
				return makeRange(Math.min(r.min, r2.min), Math.max(r.max, r2.max));
			},
			minTranslation: function minTranslation(r2) {
				// Min amount that r2 would have to shift to get off of r
				if (!r.overlaps(r2)) {
					return 0;
				}
				var dPos = r2.max - r.min;
				var dNeg = r2.min - r.max;
				return dPos < -dNeg ? dPos : dNeg;
			}
		};
		return r;
	}
...
/***/ function(module, exports, __webpack_require__) {
	...
	function makeDynamic(object) {
		...
		var dispatchCollision = object.collisionListener.getListener();
		// f: Function that takes in an {mtv, velocity} object
		object.subscribeCollisionListener = function (f) {
			return object.collisionListener.subscribe(f);
		};
		object.unsubscribeCollisionListener = function (i) {
			return object.collisionListener.unsubscribe(i);
		};

		object.vel = (0, _geom.vec)(0, 0);
		object.rvel = 0;
		object.accel = (0, _geom.vec)(0, 0);
		object.raccel = 0;
		object.properties = {
			// Response to gravity
			buoyancy: 1,
		...
		object.update = function (dt) {
			...
			(0, _lodash.forEach)(object.state.collisions, function (coll) {
				return dispatchCollision(coll);
			});
			object.state.collisions = [];

			// Add gravity
			var gravityVec = (0, _geom.vec)(0, -_physics_settings.physicsSettings.gravity).mul(object.properties.buoyancy);
			...
			object.vel = object.vel.limit(_physics_settings.physicsSettings.velLimit);
		...
		object.standardCollisionListener = object.subscribeCollisionListener(function (coll) {
	...
	function makeWorld() {
		var world = {
			...
			update: function update(dt) {
				var effDt = dt * _physics_settings.physicsSettings.baseSpeed;

				// Update objects
				(0, _lodash.forEach)((0, _lodash.values)(world.dynamicObjects), function (o) {
					return o.update(effDt);
				});
				(0, _lodash.forEach)((0, _lodash.values)(world.characters), function (c) {
					return c.update(effDt);
				});
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.getProjectionPairs = getProjectionPairs;
	...
	function getProjectionPairs(o1, o2) {
		var normals1 = o1.getNormals(),
		    normals2 = o2.getNormals();
		var pairs = [];
		pairs = pairs.concat((0, _lodash.map)(o1.getOwnProjections(), function (p) {
			return {
				normal: p.normal,
		...
		pairs = pairs.concat((0, _lodash.map)(o2.getOwnProjections(), function (p) {
			return {
				normal: p.normal,
	...
	function colliding(o1, o2) {
		var areColliding = true;
		(0, _lodash.forEach)(getProjectionPairs(o1, o2), function (pair) {
	 ...
	 */
	function getMTV(o1, o2) {
		var minTranslationDistance = Infinity,
		    minTranslation = 0,
		    mtv = (0, _geom.vec)(0, 0);
		(0, _lodash.forEach)(getProjectionPairs(o1, o2), function (pair) {
			var currMinTranslation = pair.projection1.minTranslation(pair.projection2);
	...
	function detectAndResolveCollisionStaticDynamic(staticObj, dynamicObj) {
		if (staticObj.overlapsBoundingBox(dynamicObj)) {
			var mtv = getMTV(staticObj, dynamicObj);
			dynamicObj.move(mtv);
			dynamicObj.registerCollision((0, _collision.makeCollision)(mtv, staticObj));
			if (!mtv.equals((0, _geom.vec)(0, 0))) {
				return true;
			}
		}
		return false;
...
/***/ function(module, exports) {
	...
	exports.makeCollision = makeCollision;
	function makeCollision(mtv, objectOther) {
		return {
			mtv: mtv,
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _character_action = __webpack_require__(23);
	...
	function makeCharacter(startLoc, dim) {
		var char = {
			...
			queueAction: function queueAction(action, args) {
				char.state.actions.push((0, _character_action.makeCharacterAction)(action, args));
			},
			...
			startMoveSide: function startMoveSide(side) {
				char.state.movingSide[String(side)] = true;
				char.state.currMovingSide = side;
			},
			endMoveSide: function endMoveSide(side) {
			...
			evaluateActions: function evaluateActions() {
				(0, _lodash.forEach)(char.state.actions, function (charAction) {
					charAction.action.apply(charAction, _toConsumableArray(charAction.args));
				});
				char.state.actions = [];
			},
			update: function update(dt) {
				...
				if (char.state.justJumped) {
					...
					var jumpVel = (0, _geom.vec)(0, 1).mul(char.properties.jumpSpeed);
					char.object.addVel(jumpVel);
				}
				// Mid-jump acceleration
				if (char.state.isJumping) {
					var currJumpingAccel = char.object.vel.y > 0 ? char.properties.jumpingAccelGoingUp : char.properties.jumpingAccelGoingDown;
					char.object.addAccel((0, _geom.vec)(0, currJumpingAccel));
				}
				// Side to side motion
				if (char.state.currMovingSide) {
					// Only accelerate if the current side speed is within limits
					var currSideAccel = char.state.currMovingSide * char.object.vel.x < char.properties.maxSideSpeed ? char.state.currMovingSide * char.properties.sideAccel : 0;
					char.object.addAccel((0, _geom.vec)(currSideAccel, 0));
				} else {
					// TODO To change ground direction dynamically, this bit needs a LOT of work
					var currSideVel = char.object.vel.x;
					var currSideMotion = Math.sign(currSideVel);
					var currSideDecel = char.state.grounded ? char.properties.groundedSideDecel : char.properties.aerialSideDecel;
					if (Math.abs(currSideDecel * dt) > Math.abs(currSideVel)) {
						// Set character side vel to zero
						char.object.vel = char.object.vel.project((0, _geom.vec)(0, 1));
					} else {
						// Decelerate the character
						char.object.addAccel((0, _geom.vec)(-currSideMotion * currSideDecel, 0));
					}
				}
		...
		(0, _character_controls.assignCharacterControls)(char);

		// Sets up interactions with other objects
		char.object.subscribeCollisionListener(function (coll) {
			var mtv = coll.mtv;
			var angle = mtv.angle(),
...
/***/ function(module, exports, __webpack_require__) {
	...
	function assignCharacterControls(char) {
		...
		(0, _keybindings.subscribeKeyUpListener)(function (e) {
			if (e.key == 'ArrowUp') {
...
/***/ function(module, exports) {
	...
	exports.makeCharacterAction = makeCharacterAction;
	function makeCharacterAction(action, args) {
		return {
			action: action,
			args: args
		};
